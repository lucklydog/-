...
<%
  if(session.getAttribute("admin")==null)
  	response.sendRedirect("/DWS-pm/admin_login.jsp");
 %>
  ...
  <body>
	...
	<div id="typeTb" style="padding:2px 5px;">
		<a href="javascript:void(0)" class="easyui-linkbutton"
			...
			<a
			href="javascript:void(0)" class="easyui-linkbutton"
			iconCls="icon-remove" plain="true" onclick="deleteType();">删除</a> 
	</div>
	...
	<script type="text/javascript">
		$(function() {
			$('#typeDg').datagrid({
				...
				url : 'type/typelist', //为datagrid设置数据源
		...
		function addType() {
			$('#typeDlg').dialog('open').dialog('setTitle', '新增支部');
			$('#typeDlg').form('clear');
			urls = 'type/addType';
		}
		...
		function editType() {
			...
			//console.log(row);
			if (row) {
				$("#typeDlg").dialog("open").dialog('setTitle', '修改支部信息');
				$("#typeForm").form("load", {
					"name" : row.name
				});
				urls = "type/editType/" + row.id;
			}
		}	
		*/
		function deleteType() {
			...
			if (rows.length > 0) {
				...
				$.messager.alert('提示', '请选择要删除的支部', 'info');
			}
		}
	</script>
  </body>
